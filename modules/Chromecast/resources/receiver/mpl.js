...
var protocol;
var messageBusLookupTable = {
    'show': function (payload) {
        console.info('[' + getTime() + '] ' + className + ' | messageBus | onMessage | show');
    ...
    'hide': function (payload) {
        console.log('[' + getTime() + '] ' + className + ' | messageBus | onMessage | hide');
        if (payload['target'] === 'debug') {
    ...
    'ENABLE_CC': function (payload) {
        console.log('[' + getTime() + '] ' + className + ' | messageBus | onMessage | ENABLE_CC');
        var trackNumber = payload['trackNumber'];
        setCaption(trackNumber);
    },
    'WebVTT': function () {
        console.log('[' + getTime() + '] ' + className + ' | messageBus | onMessage | WebVTT');
        mediaPlayer.enableCaptions(false);
        mediaPlayer.enableCaptions(true, 'webvtt', 'captions.vtt');
    },
    'TTML': function () {
        console.log('[' + getTime() + '] ' + className + ' | messageBus | onMessage | TTML');
        mediaPlayer.enableCaptions(false);
        mediaPlayer.enableCaptions(true, 'ttml', 'captions.ttml');
    },
    'live': function (payload) {
        console.log('[' + getTime() + '] ' + className + ' | messageBus | onMessage | live');
        if (mediaManager) {
            mediaManager.onGetStatus(event);
        }
        liveStreaming = payload['value'] === true ? payload['value'] : false;
    },
    'maxBW': function (payload) {
        console.log('[' + getTime() + '] ' + className + ' | messageBus | onMessage | maxBW');
        maxBW = payload['value'];
    },
    'license': function (payload) {
        console.log('[' + getTime() + '] ' + className + ' | messageBus | onMessage | license');
        licenseUrl = payload['value'];
        setDebugMessage('licenseUrl', licenseUrl);
    },
    'qualityIndex': function (payload) {
        console.log('[' + getTime() + '] ' + className + ' | messageBus | onMessage | qualityIndex');
    ...
    'manifestCredentials': function (payload) {
        console.log('[' + getTime() + '] ' + className + ' | messageBus | onMessage | manifestCredentials');
        manifestCredentials = payload['value'];
        setDebugMessage('manifestCredentials', manifestCredentials);
    },
    'segmentCredentials': function (payload) {
        console.log('[' + getTime() + '] ' + className + ' | messageBus | onMessage | segmentCredentials');
        segmentCredentials = payload['value'];
        setDebugMessage('segmentCredentials', segmentCredentials);
    },
    'licenseCredentials': function (payload) {
        console.log('[' + getTime() + '] ' + className + ' | messageBus | onMessage | licenseCredentials');
        licenseCredentials = payload['value'];
        setDebugMessage('licenseCredentials', licenseCredentials);
    },
    'customData': function (payload) {
        console.log('[' + getTime() + '] ' + className + ' | messageBus | onMessage | customData');
        customData = payload['value'];
        setDebugMessage('customData', customData);
    },
    'load': function (payload) {
        console.log('[' + getTime() + '] ' + className + ' | messageBus | onMessage | load');
    },
    'notification': function (payload) {
        console.log('[' + getTime() + '] ' + className + ' | messageBus | onMessage | notification');
        kdp.sendNotification(payload['event'], [payload['data']]); // pass notification event to the player
    },
    'setLogo': function (payload) {
        console.log('[' + getTime() + '] ' + className + ' | messageBus | onMessage | setLogo');
        document.getElementById('logo').style.backgroundImage = "url(" + payload['logo'] + ")";
    },
    'setKDPAttribute': function (payload) {
        console.log('[' + getTime() + '] ' + className + ' | messageBus | onMessage | setKDPAttribute');
        kdp.setKDPAttribute(payload['plugin'], payload['property'], payload['value']);
    },
    'changeMedia': function (payload) {
        console.log('[' + getTime() + '] ' + className + ' | messageBus | onMessage | changeMedia');
    ...
    'embed': function (payload) {
        console.log('[' + getTime() + '] ' + className + ' | messageBus | onMessage | embed');
                        ...
                        "readyCallback": function (playerId) {
                            if (!playerInitialized) {
                                ...
                                var loadContent = function () {
                                    console.info('[' + getTime() + '] ' + className + ' | messageBus | onMessage | embed - Load content');
                                    if (protocol !== null) {
                                        console.info('[' + getTime() + '] ' + className + ' | messageBus | onMessage | embed - Attaching content media source');
                                        mediaPlayer.load();
                                ...
                                kdp.kBind("onContentResumeRequested", function () {
                                    console.info('[' + getTime() + '] ' + className + ' | messageBus | onMessage | embed - Ad ended');
                                    loadContent();
                                });
                                kdp.kBind("adErrorEvent", function () {
                                    console.info('[' + getTime() + '] ' + className + ' | messageBus | onMessage | embed - Ad error');
                                    loadContent();
                                });
...
var onMessage = function (event) {
    console.info('[' + getTime() + '] ' + className + ' | messageBus | onMessage | ' + JSON.stringify(event));
    setDebugMessage('messageBusMessage', event);

    var payload = JSON.parse(event['data']);
    var msgType = payload['type'];
    messageBusLookupTable[msgType](payload);
};

onload = function () {
    console.info('[' + getTime() + '] ' + className + ' | mpl | onLoad | {}');
...
function setMediaManagerEvents() {
    mediaManager.customizedStatusCallback = function (status) {
        console.info('[' + getTime() + '] ' + className + " | mediaManager | customizedStatusCallback | status = " + status);
        if (maskAdEndedIdelState && (status.playerState = cast.receiver.media.PlayerState.IDLE)) {
            console.info('[' + getTime() + '] ' + className + ' | mediaManager | customizedStatusCallback | Preventing IDLE on ad ended event, set player state to BUFFERING');
    ...
    mediaManager.onEnded = function () {
        ...
        console.info('[' + getTime() + '] ' + className + " | mediaManager | onEnded | sequenceProxy.isInSequence=" + kdp.evaluate("{sequenceProxy.isInSequence}"));
    ...
    mediaManager.onError = function (obj) {
        ...
        console.error('[' + getTime() + '] ' + className + " | mediaManager | onError | " + JSON.stringify(obj));
    ...
    mediaManager.onGetStatus = function (event) {
        ...
        console.info('[' + getTime() + '] ' + className + " | mediaManager | onGetStatus | " + JSON.stringify(event));
    ...
    mediaManager.onLoadMetadataError = function (event) {
        ...
        console.error('[' + getTime() + '] ' + className + " | mediaManager | onLoadMetadataError | " + JSON.stringify(event));
    ...
    mediaManager.onLoadMetadataLoaded = function (event) {
        console.info('[' + getTime() + '] ' + className + " | mediaManager | onLoadMetadataLoaded | " + JSON.stringify(event));
    ...
    mediaManager.onPause = function (event) {
        if (kdp.evaluate("{sequenceProxy.isInSequence}")) {
            console.info('[' + getTime() + '] ' + className + " | mediaManager | onPause | Prevent pause during ad!!!!!");
        } else {
            console.info('[' + getTime() + '] ' + className + " | mediaManager | onPause | " + JSON.stringify(event));
    ...
    mediaManager.onPlay = function (event) {
        console.info('[' + getTime() + '] ' + className + " | mediaManager | onPlay | " + JSON.stringify(event));
    ...
    mediaManager.onSeek = function (event) {
        console.info('[' + getTime() + '] ' + className + " | mediaManager | onSeek | " + JSON.stringify(event));
    ...
    mediaManager.onSetVolume = function (event) {
        console.info('[' + getTime() + '] ' + className + " | mediaManager | onSetVolume | " + JSON.stringify(event));
    ...
    mediaManager.onStop = function (event) {
        console.info('[' + getTime() + '] ' + className + " | mediaManager | onStop | " + JSON.stringify(event));
    ...
    mediaManager.onLoad = function (event) {
        broadcast("mediaManager.onLoad");

        console.info('[' + getTime() + '] ' + className + " | mediaManager | onLoad | " + JSON.stringify(event));
...
//			}
            ...
            mediaHost.onError = function (errorCode, requestStatus) {
                broadcast("mediaHostState: Fatal Error: code = " + errorCode);

                console.error('[' + getTime() + '] ' + className + " | mediaHost | onError | Fatal Error: code = " + errorCode);
                setDebugMessage('mediaHostState', 'Fatal Error: cde = ' + errorCode);
            ...
            console.info('[' + getTime() + '] ' + className + " | mediaManager | onLoad | Media Protocol Identified as " + ext);
            ...
            broadcast("mediaHostState: success");
...
function initApp() {
    console.info('[' + getTime() + '] ' + className + " | mpl | initApp | Application Loaded. Starting system.");
    ...
    castReceiverManager.onShutdown = function () {
        broadcast("shutdown"); // receiver was shut down by the browser Chromecast icon - send message to the player to stop the app
    };
    /**
...
function setCastReceiverManagerEvents() {
    castReceiverManager.onReady = function (event) {
        console.info('[' + getTime() + '] ' + className + " | castReceiverManager | onReady | " + JSON.stringify(event));
    ...
    castReceiverManager.onSenderConnected = function (event) {
        console.info('[' + getTime() + '] ' + className + " | castReceiverManager | onSenderConnected | " + JSON.stringify(event));
    ...
    castReceiverManager.onSenderDisconnected = function (event) {
        console.info('[' + getTime() + '] ' + className + " | castReceiverManager | onSenderDisconnected | " + JSON.stringify(event));
    ...
    castReceiverManager.onSystemVolumeChanged = function (event) {
        console.info('[' + getTime() + '] ' + className + " | castReceiverManager | onSystemVolumeChanged | " + JSON.stringify(event));
        setDebugMessage('castReceiverManagerMessage', 'System Volume Changed: ' + JSON.stringify(event));
        // See cast.receiver.media.Volume
        console.info('[' + getTime() + '] ' + className + " | castReceiverManager | onSystemVolumeChanged | Volume: " + event.data['level'] + ' is muted? ' + event.data['muted']);
...
function setMediaElementEvents(mediaElement) {
    mediaElement.addEventListener('loadstart', function (e) {
        ...
        console.info('[' + getTime() + '] ' + className + " | mediaElement | loadstart | {}");
        setDebugMessage('mediaElementState', 'Load Start');

        broadcast("mediaElement: Load Start");
    });
    ...
    mediaElement.addEventListener('loadeddata', function (e) {
        ...
        console.info('[' + getTime() + '] ' + className + " | mediaElement | loadeddata | {}");
        setDebugMessage('mediaElementState', 'Data Loaded');

        broadcast("mediaElement:Data Loaded");
        ...
        console.info('[' + getTime() + '] ' + className + " | mediaElement | loadeddata | send captions to senders : " + JSON.stringify(captions));
    ...
    mediaElement.addEventListener('seeking', function (e) {
        console.info('[' + getTime() + '] ' + className + " | mediaElement | seeking | " + e);
        setDebugMessage('mediaElementState', 'Seeking');
        getPlayerState();
    });

    mediaElement.addEventListener('seeked', function (e) {
        console.info('[' + getTime() + '] ' + className + " | mediaElement | seeked | " + e);
